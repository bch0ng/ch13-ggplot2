...
source('data-prep.R')
colnames(all.data)[1] <- "County.name"
# Create a column of percentage of registered voters who voted
all.data <- all.data %>% 
            mutate(pct.voted = total.voted / registered.voters * 100)
...
ggplot(data = all.data, aes(x = total.population, y = pct.voted)) +
  geom_text(mapping = aes(label = County.name), color = 'blue')
# Draw a scatter plot comparing the female percentage to the percent voted
# Size the points by the population of the county
ggplot(data = all.data, aes(x = female.percentage, y = pct.voted)) +
  geom_point(aes(size = total.population))

# Draw a line graph comparing the total population to the percent male
# Make it a green line
ggplot(data = all.data, aes(x = total.population, y = male.percentage)) +
